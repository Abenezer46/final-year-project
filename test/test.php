...
if (isset($_POST['update'])) {
    ...
    if ($quant >= 1) {
        ...
        header('Location: cart.php?error= quantity must be above 1');
    }
}
// remove from cart
...
if (isset($_GET['sell'])) {
    ...
    if ($result && mysqli_affected_rows($con) > 0) {
        ...
        //     $sql = "UPDATE store SET quantity = quantity - $qu WHERE id = $itemid";
        //     $update_result = mysqli_query($con, $sql);

        //     if ($update_result && mysqli_affected_rows($con) > 0) {
        //         // Check if the updated quantity is less than zero
        //         $sql = "SELECT quantity FROM store WHERE id = $itemid";
        //         $check_result = mysqli_query($con, $sql);

        //         echo "select item";
        //         if ($check_result && mysqli_affected_rows($con) > 0) {
        ...
        //                 echo "Error: Quantity cannot be less than zero.";
        //                 break;
        //             }
        //         }
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $sql = "SELECT quantity FROM store WHERE id = $itemid";
            $getquant = mysqli_query($con, $sql);

            if ($getquant && mysqli_affected_rows($con) > 0) {
                $row = mysqli_fetch_assoc($check_result);
...
<body class="sellerPage p-3 m-0 border-0 bd-example m-0 border-0" style="background-color: #585BB8;">
    ...
    <div class="container py-5">
        ...
        <div>
            ...
            <a href="test.php?sell" class="btn btn-outline-success">
                done
            </a>
